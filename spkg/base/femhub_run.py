...
def install_package_spkg(pkg):
    ...
    cmd("cd $FEMHUB_ROOT/spkg/build/%s-%s; chmod +x spkg-install" % (name, version))
    cmd("cd $FEMHUB_ROOT/spkg/build/%s-%s; . $FEMHUB_ROOT/local/bin/femhub-env; ./spkg-install" % (name, version))
